// ==UserScript==
// @name        netcool clip
// @namespace   email@gmail.com
// @version     1.0
// @description		ALL UR CLIPBOARD ARE BELONG TO US  -  copy2clipboard hack - thank you, you are making our job easier
// @include https://xxx/ibm/console/contentRender.do*
// @include https://xxx/ibm/console/contentRender.do*
// @include https://xxx:111/ibm/console/contentRender.do*
// @include https://xxx:111/ibm/console/contentRender.do*
 ...
 * <email@gmail.com> wrote this file. As long as you retain this notice you
...
0.4 - ibtool img in btn; better handling of hostnames
0.5 - prevent to change fincantieri IP if it is already the right one (106.)
0.6 - no more scrollbars (resize window to see license ;)
0.7 - wcd_cdp_tsm_ibm -> cdp_tsm_ibm
0.8 - added SEVerity & GROUP to ibtool query - MTS - Requests DB: MTS Request ID: 32721 -> https://bat.brno-centrum.cz.ibm.com/idcmts/redirect.php?tool=15&page=requestView&id=32721
0.9 - new year's bug - ibtool query should contain actual date not hard-coded year
1.0 - OUTAGE button added; renamed "capoturno" btn to "mail"

-- DONT FORGOT TO UPDATE VARIABLE! --
*/
...
enablePrivilege("UniversalXPConnect") was removed in later version of GECKO so in case they will update Citrix/Netcool version of Firefox (now 3.6.8)
use GreaseMonkey's setClipboard() or if you can/want use another browser check some Flash copy-to-clipboard implementation
or HTML5 clipboard api: http://www.w3.org/TR/clipboard-apis/ but it's Fx 33+ http://caniuse.com/#feat=clipboard
...
user_pref("browser.startup.homepage", "https://xxx:111/ibm/console/logon.jsp");
user_pref("browser.startup.homepage_override.mstone", "rv:1.9.2.28");
user_pref("capability.principal.codebase.p0.granted", "UniversalXPConnect");
user_pref("capability.principal.codebase.p0.id", "https://xxx:111");
user_pref("capability.principal.codebase.p0.subjectName", "");
user_pref("capability.principal.codebase.p1.granted", "UniversalXPConnect");
user_pref("capability.principal.codebase.p1.id", "https://xxx:111");
...
location.replace("javascript:(" + function() {
	...
	var ver = "1.0"; // change it everywhere in code (changelog + header)
	var node;
	var nodeIP;
	var group;
	var summary;
		...
		while(i--) {
		    ...
		    switch ( b[i].textContent.trim() ){
		        case "Node":
		            ...
		            after gathering enough data, refactor
		            zatím to vypadá že stačí počekovat všechno mezi XXX_HOSTNAME-cokoliv-dokud-neni-jedno-pismenko||cluster
		            */
		            //full_node=val;
		            arr = val.split("_")
		            nodeWithoutCustomer = val.slice(val.indexOf("_")+1);
		            customer_prefix = arr[0]
	...
	function copy(what,ip) {
		if (wfi && ip && checkForFincantieri == true) {
			// maybe they fixed it, but I spotted CUSTOMER with 106. IP address
			if (what[2] != "6") {
				// 11.1.44.123 -> 116.1.44.123
	...
	function ib_copy() {
		//not hard-coded, now is 1.1.NEXT-YEAR and it was not workin ;)
		year = new Date().getFullYear();
		// required time format - one-liner
		time = new Date().toString().split("GMT")[0].replace(" "+year,"")+year;
		...
		wci_tsm_pero      -> tsm_pero
		dirty hack - should work: .split("_")[0]  still you need to correct filled values..user will choose in ibtool offered assets
		*/
		//str = node.split("_")[0] +"^"+ summary +"^^"+ arr[0] +"^^^^^^["+ time +"]^^"+group+"^"+sev;
		 // => wci_tsm_pero^TSM server TSM_COIN_PERO:N/A too few scratch lib volumes:9^^wci^^^^^^[Fri Dec 26 03:12:22 2014]^^TSM_MONITORING_NCC^3
		 //str = customer_prefix+"_"+node+"^"+summary+"^^"+customer_prefix+"^^^^^^["+ time +"]^^"+group+"^"+sev;
		// => wci_tsm^TSM server TSM_COIN_PERO:N/A too few scratch lib volumes:9^^wci^^^^^^[Fri Dec 26 03:13:11 2014]^^TSM_MONITORING_NCC^3
		str = customer_prefix+"_"+node.split("_")[0]+"^"+summary+"^^"+customer_prefix+"^^^^^^["+ time +"]^^"+group+"^"+sev;
		cp( str );
	}

	// TODO: rich-text formatting? BOLD?  \b turns on bold, whereas \b0
	// looks like it's impossible; possible way mailto:// or just link to Notes:// so we can open stationary, but without values..
	var address = "mail1@example.com";
	var cc = "DISTRIBUTED/Czech Republic/IBM";
	...
	var outage = "" +
		...
		"Severity:  "                +sev                   +"\n" +
		"Customer:  "                +customer              +"\n" +
		"Hostname:  "                +nodeWithoutCustomer   +"\n" +
		"IP:  "                      +nodeIP                +"\n" +
		"Description:  "             +summary               +"\n\n"+
		"Please manage and let us know.\n\n";

	var elements = ['hostname', 'ip', 'group', 'summary', 'ibtool','mail','info','agent','OUT'].reverse();
	...
	document.getElementById('mail').addEventListener     ("click", function(){ location.href = mailto  }, true);
	document.getElementById('info').addEventListener     ("click", function(){ cp("BlueCARE Netcool Event Information\n\n"+document.getElementsByTagName("pre")[0].innerHTML.replace(/<b>|<\/b>|<null>|<\/null>/g,"").replace(/<br>/g,"\n"))    }, true); // I saw <null> pair tag in one alert (bug on their side)
